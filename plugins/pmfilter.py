...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    # Document and Video type handlers
    elif query.data.startswith("doctype"):
        file_id = query.data.split("#")[1]
        if not hasattr(temp, 'FILE_DATA') or file_id not in temp.FILE_DATA:
            return await query.answer('File data not found. Please try again.', show_alert=True)
            
        file_data = temp.FILE_DATA[file_id]
    ...
    elif query.data.startswith("videotype"):
        file_id = query.data.split("#")[1]
        if not hasattr(temp, 'FILE_DATA') or file_id not in temp.FILE_DATA:
            return await query.answer('File data not found. Please try again.', show_alert=True)
        ...
        try:
            ...
            poster_url = None
            try:
                # Extract movie name from title (remove quality, year, etc.)
                movie_name = file_data["title"].split("(")[0].strip() if "(" in file_data["title"] else file_data["title"]
                # Remove extensions like .mkv, .mp4, etc.
                ...
                logger.exception(f"Error getting IMDB poster: {e}")
                
            # Apply watermark if settings exist and we have a poster or file cover
            if (watermark_text or file_cover) and (poster_url or file_cover):
                try:
                    ...
                    if image_path:
                        ...
                        # Fallback to regular send if image processing fails
                    ...
                    logger.exception(f"Error processing watermark: {e}")
                    # Fallback to regular send if watermarking fails
                    await client.send_cached_media(
                        chat_id=query.from_user.id,
                        file_id=file_data["file_id"],
                        caption=file_data["caption"] + (f"\n\n{watermark_text}" if watermark_text else ""),
                ...
                # Send as video without watermark
                await client.send_cached_media(
                    chat_id=query.from_user.id,
                    file_id=file_data["file_id"],
                    caption=file_data["caption"] + (f"\n\n{watermark_text}" if watermark_text else ""),
            ...
            await query.answer('File sent as video!', show_alert=True)
        ...
        # Optimize by using asyncio.gather to run all searches and send_all operations concurrently
        search_tasks = []
        if search:
            search_tasks.append(get_search_results(query.message.chat.id, search, offset=int(offset), filter=True))
        
        search1 = BUTTONS1.get(key)
        if search1:
            search_tasks.append(get_search_results(query.message.chat.id, search1, offset=int(offset), filter=True))
            
        search2 = BUTTONS2.get(key)
        if search2:
            search_tasks.append(get_search_results(query.message.chat.id, search2, offset=int(offset), filter=True))
        ...
        for result in search_results:
            files, _, _ = result
            if files:
                send_tasks.append(send_all(client, query.from_user.id, files, ident, query.message.chat.id, query.from_user.first_name, query))
        ...
        # Optimize by using asyncio.gather for search and send operations
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            # Process search string in parallel with getting settings
            async def process_search_string():
                find = search.split(" ")
                processed_search = ""
                ...
                for x in find:
                    if x not in removes:
                        processed_search += x + " "
                processed_search = processed_search.replace("-", " ").replace(":","")
                return processed_search
            
            # Run search processing and settings retrieval in parallel
            processed_search, settings = await asyncio.gather(
                process_search_string(),
                get_settings(message.chat.id)
            )
            
            # Get search results with processed search string
            files, offset, total_results = await get_search_results(message.chat.id, processed_search, offset=0, filter=True)
            
            if not files:
                if settings["spell_check"]:
                    ai_sts = await m.edit('ᴘʟᴇᴀꜱᴇ ᴡᴀɪᴛ, ʟᴜᴄʏ ɪꜱ ᴄʜᴇᴄᴋɪɴɢ ʏᴏᴜʀ ꜱᴘᴇʟʟɪɴɢ...')
                    is_misspelled = await ai_spell_check(chat_id = message.chat.id, wrong_name=processed_search)
                    if is_misspelled:
