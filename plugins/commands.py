...
async def start(client, message):
    ...
    if len(message.command) != 2:
        ...
        if curr_time < 12:
            ...
            gtxt = "ɢᴏᴏᴅ ɴɪɢʜᴛ 👋"
        # Removed animation sequence to make bot faster and more efficient
...
async def set_watermark_text(client, message):
    """Set watermark text to be placed at the bottom of thumbnails"""
    try:
        # Check if there's text after the command
        if len(message.command) < 2:
            await message.reply_text("⚠️ Please provide the watermark text after the command.\nExample: /setwatermark Your Channel Name")
            return
            
        # Get the watermark text
        watermark_text = message.text.split(None, 1)[1].strip()
        
        # Import watermark_db here to avoid circular imports
        ...
        if success:
            ...
            await message.reply_text("❌ Failed to set watermark text. Please try again.")
            
    except Exception as e:
        await message.reply_text(f"❌ An error occurred: {str(e)}")
        await log_error(client, f"Error in set_watermark_text: {e}")

@Client.on_message(filters.command("setwatermarkusername") & filters.user(ADMINS))
async def set_watermark_username(client, message):
    """Set watermark username to be appended to file names"""
    try:
        # Check if there's text after the command
        if len(message.command) < 2:
            await message.reply_text("⚠️ Please provide the username text after the command.\nExample: /setwatermarkusername @YourChannel")
            return
            
        # Get the watermark username
        watermark_username = message.text.split(None, 1)[1].strip()
        ...
        if success:
            ...
            await message.reply_text("❌ Failed to set watermark username. Please try again.")
            
    except Exception as e:
        await message.reply_text(f"❌ An error occurred: {str(e)}")
        await log_error(client, f"Error in set_watermark_username: {e}")

@Client.on_message(filters.command("setfilecover") & filters.user(ADMINS))
async def set_file_cover(client, message):
    """Set file cover (logo) to be placed on thumbnails"""
    try:
        # Check if there's a photo attached or replied to
        if message.photo:
            photo = message.photo.file_id
        elif message.reply_to_message and message.reply_to_message.photo:
            photo = message.reply_to_message.photo.file_id
        else:
            await message.reply_text("⚠️ Please attach a photo or reply to a photo with this command to set it as the file cover.")
            return
        ...
        if success:
            ...
            await message.reply_text("❌ Failed to set file cover. Please try again.")
            
    except Exception as e:
        await message.reply_text(f"❌ An error occurred: {str(e)}")
        await log_error(client, f"Error in set_file_cover: {e}")

@Client.on_message(filters.command("clearwatermark") & filters.user(ADMINS))
async def clear_watermark(client, message):
    """Clear all watermark settings"""
    try:
        # Import watermark_db here to avoid circular imports
        from database.watermark_db import watermark_db
        ...
        await message.reply_text(f"❌ An error occurred: {str(e)}")
        await log_error(client, f"Error in clear_watermark: {e}")

@Client.on_message(filters.command("watermarkstatus") & filters.user(ADMINS))
async def watermark_status(client, message):
    """Show current watermark settings"""
    try:
        # Import watermark_db here to avoid circular imports
        from database.watermark_db import watermark_db
        ...
        status_text += f"<b>Watermark Username:</b> {watermark_username or 'Not set'}\n"
        status_text += f"<b>File Cover:</b> {'Set' if file_cover else 'Not set'}\n\n"
        status_text += "<b>Commands:</b>\n"
        status_text += "/setwatermark - Set text for thumbnail\n"
        status_text += "/setwatermarkusername - Set username for filenames\n"
        status_text += "/setfilecover - Set logo for thumbnails\n"
        status_text += "/clearwatermark - Clear all settings"
        
        # If file cover is set, send it with the status
        if file_cover:
            await message.reply_photo(
                photo=file_cover,
                caption=status_text
        ...
        await message.reply_text(f"❌ An error occurred: {str(e)}")
        await log_error(client, f"Error in watermark_status: {e}")
