...
# Optimized send_all function - removed since SEND ALL button was removed
async def send_all(bot, userid, files, ident, chat_id, user_name, query):
    # This function is kept for backward compatibility but optimized
    # since the SEND ALL button has been removed from the UI
    try:
        settings = await get_settings(chat_id)
        ENABLE_SHORTLINK = settings.get('is_shortlink', False)
        
        # Use asyncio.gather to send all files concurrently for better performance
        tasks = []
        for file in files:
            ...
            if ENABLE_SHORTLINK:
                shortlink = await get_shortlink(chat_id, f"https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}")
                message = f"<b>Hᴇʏ ᴛʜᴇʀᴇ {user_name} 👋🏽 \n\n✅ Sᴇᴄᴜʀᴇ ʟɪɴᴋ ᴛᴏ ʏᴏᴜʀ ғɪʟᴇ ʜᴀs sᴜᴄᴄᴇssғᴜʟʟʏ ʙᴇᴇɴ ɢᴇɴᴇʀᴀᴛᴇᴅ ᴘʟᴇᴀsᴇ ᴄʟɪᴄᴋ ᴅᴏᴡɴʟᴏᴀᴅ ʙᴜᴛᴛᴏɴ\n\n🗃️ Fɪʟᴇ Nᴀᴍᴇ : {title}\n🔖 Fɪʟᴇ Sɪᴢᴇ : {size}</b>"
                markup = InlineKeyboardMarkup([[InlineKeyboardButton("📤 Dᴏᴡɴʟᴏᴀᴅ 📥", url=shortlink)]])
                tasks.append(bot.send_message(chat_id=userid, text=message, reply_markup=markup))
            else:
                ...
                tasks.append(bot.send_cached_media(
                    chat_id=userid,
        ...
        await query.answer('Hᴇʏ, Sᴛᴀʀᴛ Bᴏᴛ Fɪʀsᴛ', show_alert=True)
    except Exception as e:
        logger.error(f"Error in send_all: {str(e)}")
