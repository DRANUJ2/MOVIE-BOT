...
async def Lucy_start():
    print('\n')
    print('\nInitalizing Lucy')
    # Start bot and initialize clients concurrently
    ...
    # Run multiple async tasks concurrently for better performance
    tasks = []
    
    if ON_HEROKU:
        tasks.append(ping_server())
    
    # Database operations
    tasks.extend([
        db.get_banned(),
        Media.ensure_indexes(),
        Media2.ensure_indexes(),
    ...
    # Skip ping_server result if ON_HEROKU is True
    if ON_HEROKU:
        task_index += 1
    
    # Process banned users and chats
    b_users, b_chats = results[task_index]
    temp.BANNED_USERS = b_users
    temp.BANNED_CHATS = b_chats
    task_index += 1
    
    # Skip Media.ensure_indexes and Media2.ensure_indexes results
    task_index += 2
    
    # Process database stats
    stats = results[task_index]
